...
export default function ObserverView({
  ...
  const myBet =
    bets.find(
      (b) => b.market_id === market?.id && b.participant_id === participantId
    ) ?? null;
  ...
  // Load all participants (leaderboard) on mount
  useEffect(() => {
    supabase
      .from("participants")
      ...
      .then(({ data }) => {
        ...
        setParticipants(
          data.map((p) => ({
            ...
            user_id: p.user_id,
            display_name:
              (p.users as unknown as { display_name: string } | null)
                ?.display_name ?? p.id.substring(0, 8),
            bankroll: p.bankroll,
          }))
        );
        const me = data.find((p) => p.id === participantId);
        if (me) setBankroll(me.bankroll);
      });
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  // Load bets when market changes
  useEffect(() => {
    if (!market) return;
    supabase
      .from("bets")
      ...
      .then(({ data }) => {
        if (data) setBets(data as BetRow[]);
      });
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [market?.id]);
    ...
    const tick = () =>
      setSecondsLeft(
        Math.max(
          0,
          Math.round(
            (new Date(market.closes_at).getTime() - Date.now()) / 1000
          )
        )
      );
    tick();
    ...
    const channel = supabase
      ...
      .on(
        ...
        async (payload) => {
          if (payload.eventType === "INSERT") {
            ...
            setBets([]);
            addLog(`Market opened line=${m.line}`);
          } else if (payload.eventType === "UPDATE") {
            const updated = payload.new as MarketRow;
            addLog(
              `Market → ${updated.status}${updated.resolved_outcome ? ` (${updated.resolved_outcome})` : ""}`
            );
      ...
      .on(
        ...
        (payload) => {
          ...
          setParticipants((prev) =>
            prev.map((p) =>
              p.id === updated.id ? { ...p, bankroll: updated.bankroll } : p
            )
          );
          if (updated.id === participantId) setBankroll(updated.bankroll);
          const name =
            participants.find((p) => p.id === updated.id)?.display_name ??
            updated.id.substring(0, 8);
      ...
      .on(
        ...
        (payload) => {
          ...
          setBets((prev) => [
            ...prev.filter((b) => b.id !== inserted.id),
            inserted,
          ]);
          addLog(`Bet: ${inserted.selection} ${inserted.stake} coins`);
        }
      )
      .subscribe((status) => {
  ...
  async function placeBet(selection: "over" | "under") {
    ...
    if (!res.ok || json.error) {
      ...
      setBankroll(json.bankroll as number);
    }
    setBetting(false);
  }
  ...
  async function fetchLatest() {
    ...
    if (data) {
      ...
      addLog(`Fetched market status=${data.status}`);
  ...
  const canBet = isOpen && !myBet && (secondsLeft ?? 1) > 0;

  return (
    <main style={{ fontFamily: "monospace", padding: "1rem", maxWidth: 800 }}>
      <h1 style={{ marginBottom: "0.25rem" }}>Live Bet — Observer</h1>
      <p style={{ color: "#888", marginBottom: "1rem" }}>
        {displayName} · bankroll: {bankroll} coins
      </p>
      ...
      >
        ...
        <button
          onClick={async () => {
            const res = await fetch("/api/dev/close-markets", {
              method: "POST",
            });
            const json = await res.json();
            ...
            addLog(
              n === 0
                ? "0 markets closed — closes_at still in the future"
                : `${n} market(s) resolved`
            );
          }}
          style={{ padding: "0.4rem 0.8rem" }}
      ...
      {market && (
        ...
        >
          <div style={{ marginBottom: "0.5rem" }}>
            ...
            <span
              style={{
                color: statusColor[market.status] ?? "#888",
                fontWeight: "bold",
              }}
            >
            ...
            >
              {myBet ? (
                <span style={{ color: "#888" }}>
                  Your bet: <strong>{myBet.selection}</strong> · {myBet.stake}{" "}
                  coins
              ...
              {bets.map((b) => {
                const name =
                  participants.find((p) => p.id === b.participant_id)
                    ?.display_name ?? b.participant_id.substring(0, 8);
                return (
                  <div key={b.id}>
                    {name}: {b.selection} · {b.stake} coins
                    {b.payout !== null ? ` → payout ${b.payout}` : ""}
                  </div>
                );
              })}
            </div>
